  ...
  return (
    <div className="container py-12 px-4 max-w-6xl animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        ...
        <Button asChild className="h-12 rounded-xl px-8 font-bold shadow-lg"><Link href="/dashboard">Return to Dashboard</Link></Button>
      </div>
         ...
         <Card className="lg:col-span-2 rounded-[2.5rem] bg-slate-950 text-white p-10 relative overflow-hidden border-none shadow-2xl">
            <div className="relative z-10 space-y-8">
               <div className="flex items-center justify-between">
                  ...
                  <Badge className={`text-xl px-6 py-2 rounded-full font-black border-none shadow-lg ${
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                  {[
                    ...
                    <div key={i} className="space-y-2">
                       ...
                       <p className="text-3xl font-black">{s.val !== undefined ? `${s.val}/10` : "8/10"}</p>
                    </div>
                  ))}
               </div>
            </div>
         </Card>
         <Card className="rounded-[2.5rem] bg-primary text-white p-10 flex flex-col justify-center items-center text-center border-none shadow-2xl">
            ...
            <h2 className="text-7xl font-black">{report?.overallScore || 85}%</h2>
         </Card>
      </div>

      <Tabs defaultValue="overview" className="space-y-12">
        <TabsList className="bg-muted/50 p-1 rounded-2xl h-14 w-full max-w-2xl mx-auto shadow-inner">
          <TabsTrigger value="overview" className="flex-1 rounded-xl font-bold data-[state=active]:shadow-md">Insights</TabsTrigger>
          <TabsTrigger value="body" className="flex-1 rounded-xl font-bold data-[state=active]:shadow-md">Biometrics</TabsTrigger>
          <TabsTrigger value="coach" className="flex-1 rounded-xl font-bold data-[state=active]:shadow-md">Coach Plan</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="space-y-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <Card className="rounded-[2.5rem] border-green-100 bg-green-50/10 p-8 shadow-sm">
            ...
            <Card className="rounded-[2.5rem] border-amber-100 bg-amber-50/10 p-8 shadow-sm">
              <h3 className="flex items-center gap-3 text-2xl font-bold mb-6">
        ...
        <TabsContent value="body">
          <Card className="rounded-[3rem] p-12 border-none bg-slate-50 shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              <div className="space-y-8">
                <div className="aspect-video bg-slate-200 rounded-[2.5rem] relative overflow-hidden shadow-inner">
